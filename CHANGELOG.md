...
- **Sağlık notları — klinik ölçüm alanları**: Tansiyon (sistolik/diastolik mmHg),
  kan şekeri (mg/dL), ağrı seviyesi (0–10 slider), semptom hızlı seçimi (12 hazır
  semptom) ve günlük ilaç alımı takibi not editörüne eklendi
- **Sağlık notu kart görünümü**: Ölçüm rozeti, semptom etiketleri ve "ilaç
  alınmadı" uyarısı not listesinde doğrudan gösterilir
- **Trend grafikleri**: `fl_chart ^0.70.2` bağımlılığıyla tansiyon
  (sistolik/diastolik, çift çizgi) ve kan şekeri ölçümleri için çizgi grafik
  görünümü eklendi
- **"Doktora Göster" modu**: Klinik özet, ölçüm trendleri, ağrı takibi ve semptom
  sıklığını bir araya getiren paylaşılabilir `DraggableScrollableSheet` modal
